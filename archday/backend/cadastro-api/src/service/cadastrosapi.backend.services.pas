unit cadastrosapi.backend.services;
...
uses
  ...
  DataSet.Serialize,
  cadastrosapi.backend.resource.connect;
...
begin
  ...
  FIndexConn := cadastrosapi.backend.resource.connect.Connected;
  FConn := TSimpleQueryFiredac.New(cadastrosapi.backend.resource.connect.FConnList.Items
    [FIndexConn]);
...
begin
  FDataSource.Free;
  cadastrosapi.backend.resource.connect.Disconnected(FIndexConn);
  inherited;
end;
