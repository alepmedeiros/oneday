unit autenticacaoapi.backend.services;
...
uses
  ...
  autenticacaoapi.backend.resource.connect;

type
  iServices<T: class> = interface
    function Find(aId: Integer): T;
    function FindAll: TObjectList<T>;
    function FindWhere(aKey: String; aValue: Variant): T;
...
begin
  ...
  FIndexConn := autenticacaoapi.backend.resource.connect.Connected;
  FConn := TSimpleQueryFiredac.New(autenticacaoapi.backend.resource.connect.FConnList.Items
    [FIndexConn]);
...
begin
  FDataSource.Free;
  autenticacaoapi.backend.resource.connect.Disconnected(FIndexConn);
  inherited;
end;
